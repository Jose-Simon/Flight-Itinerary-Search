...
type Props = {
  ...
  onSave: (name: string, filters: FilterSnapshot) => void
  onUpdate: (id: string, filters: FilterSnapshot) => void
...
export function FilterPresetsBar({
  ...
  function handleSaveClick() {
    setSaveName('')
    if (selected) {
      // A preset is active — ask whether to replace or save as new
  ...
  function handleConfirmReplace() {
    if (!selected) return
    onUpdate(selected.id, currentFilters)
    setSavingMode(null)
  }
  ...
  function handleStartRename(p: FilterPreset) {
    setRenaming(p.id)
  ...
  function handleToggleDefault(p: FilterPreset) {
  ...
  return (
    <div className="filter-presets-bar">
      <div className="filter-presets-row">
        <span className="filter-presets-label">Filters</span>
        ...
        {savingMode === null && (
          <button
            ...
            title={selected ? `Save current filters — will ask to replace "${selected.name}" or save as new` : 'Save current filters as a new preset'}
          >
            + Save current
          </button>
        )}

...
type Props = {
  presets: DatePreset[]
  currentDates: DateSnapshot
  onApply: (dates: DateSnapshot) => void
  onSave: (name: string, dates: DateSnapshot) => void
  onUpdate: (id: string, dates: DateSnapshot) => void
...
export function DatePresetsBar({
  presets,
  currentDates,
  onApply,
  onSave,
  ...
  function handleConfirmReplace() {
    if (!selected) return
    onUpdate(selected.id, currentDates)
    setSavingMode(null)
  }
  ...
  function handleCommitSave() {
    ...
    if (!name) return
    onSave(name, currentDates)
    setSavingMode(null)
    setSaveName('')
  }

  function handleStartRename(p: DatePreset) {
    setRenaming(p.id)
  ...
  function handleToggleDefault(p: DatePreset) {
  ...
  return (
    <div className="filter-presets-bar">
      <div className="filter-presets-row">
        <span className="filter-presets-label">Dates</span>
        ...
        {savingMode === null && (
          <button
            ...
            title={selected ? `Save current dates — will ask to replace "${selected.name}" or save as new` : 'Save current dates as a new preset'}
          >
            + Save current
          </button>
        )}
